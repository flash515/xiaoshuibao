    ...
    if (this.data.investorname == "" || this.data.investorname == null) {
      ...
      if (this.data.imageuploadlock) {
        ...
        for (let i = 0; i < this.data.attachmentview.length; i++) {
          ...
          const cloudPath = app.globalData.Gopenid + '/order/soleproprietor/' + this.data.investorname + '/' + this.data.investorname + 'ORDER' + (new Date()).getTime() + filePath.match(/\.[^.]+?$/)
          wx.cloud.uploadFile({
            cloudPath,
            filePath,
            success: res => {
